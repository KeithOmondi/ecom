                ...
                <Dialog open={showCheckout} onClose={() => setShowCheckout(false)} className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
                    <Dialog.Panel className="bg-white p-6 rounded-lg shadow-lg w-96">
                        ...
                        <div className="mt-4 space-y-3">
                            <button className="w-full px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded transition" onClick={() => setPaymentMethod("mpesa")}>Pay via Mpesa</button>
                            <button className="w-full px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded transition" onClick={() => setPaymentMethod("bank")}>Pay via Bank</button>
                        </div>
                        {paymentMethod === "mpesa" && (
                            <div className="mt-4">
                                <label className="block text-sm font-bold text-gray-700">Enter Mpesa Number</label>
                                <input type="text" className="w-full p-2 border rounded mt-1 focus:ring focus:ring-blue-300" value={mpesaNumber} onChange={(e) => setMpesaNumber(e.target.value)} placeholder="e.g. 0712345678" />
                        ...
                        {paymentMethod === "bank" && (
                            <div className="mt-4">
                                ...
                                <select className="w-full p-2 border rounded mt-1 focus:ring focus:ring-yellow-300" value={bank} onChange={(e) => setBank(e.target.value)}>
                                ...
                                <input type="number" className="w-full p-2 border rounded mt-1 focus:ring focus:ring-yellow-300" placeholder="Amount in KES" />
                                <button className="mt-2 w-full bg-yellow-500 hover:bg-yellow-600 text-white py-2 rounded transition">Pay</button>
                            </div>
                        )}
                        <button className="mt-4 w-full bg-gray-400 hover:bg-gray-500 text-white py-2 rounded transition" onClick={() => setShowCheckout(false)}>Close</button>
                    </Dialog.Panel>
                </Dialog>
            </div>
        </>
